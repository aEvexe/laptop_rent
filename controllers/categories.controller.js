...
const create = (req, res) => {
    const { error, value } = brandValidation(req.body) 
        if(error) {
            console.log(`Error crating new category`, error);
        }
    ...
    db.query(sql, [name], (err, result) => {
        if (err) {
          console.log(`Error crating new category`, err);
...
const update = (req, res) => {
    ...
    db.query(`UPDATE categories SET name=? WHERE id =?`, [...values, id], (err, result) => {
        if(err){
            res.status(500).send({message: `${err.message}`});
        }
        res.status(200).send({message: "category updated succesfully"}, result)
    })
}
